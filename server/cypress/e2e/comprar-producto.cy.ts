...
describe("Flujo E2E: Realizar una compra completa", () => {
  ...
  it("Debe completar el flujo desde el login hasta el pago", () => {
    ...
    cy.get("input")
      .filter('[type="email"], [name*="email"], [placeholder*="Correo"]')
      .first()
      .should("be.visible")
      .type("cristianmmst@gmail.com", { force: true });

    // input de contraseña
    cy.get('input[type="password"]')
      .should("be.visible")
      .type("melannie123", { force: true });
    ...
    cy.get('a[href*="cart"], button[class*="cart"], .cart-icon')
      .first()
      .click();
